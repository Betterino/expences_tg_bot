...
from texts import CATEGORY_ADDED,CATEGORY_RENAMED, NAME_EXISTS,CATEGORY_DELETED,ASK_NEW_NAME,CHOOSE_ARCHIVE,ASK_MAX,DELETE_CONFIRM,BAD_AMOUNT,CATEGORY_MAX, PROMPT_MAXIMUM, CATEGORY_ARCHIVED,CATEGORY_DEARCHIVED, EMPTY_NAME
...
async def edit_categories_create(callback: CallbackQuery, state: FSMContext, callback_data: NavCB, db: Database, budget_id):
    ...
    text = await build_edit_screen(db, budget_id, CATEGORY_ADDED.format(name=data["category_name"]),callback_data.kind)
...
async def edit_categories_rename(callback: CallbackQuery, state: FSMContext, callback_data: NavCB, db: Database, budget_id):
    ...
    await render_edit_screen(callback,db,budget_id,state,CATEGORY_RENAMED.format(name=data["category_name"]),callback_data.kind)
...
async def edit_categories_delete(callback: CallbackQuery, state: FSMContext, callback_data: NavCB, db: Database, budget_id):
    ...
    await render_edit_screen(callback,db,budget_id,state,CATEGORY_DELETED,callback_data.kind)
...
async def edit_categories_cat(callback: CallbackQuery,state: FSMContext, db: Database,callback_data: NavCB, budget_id):
    ...
    await callback.message.edit_text(text=text+"\n\n"+ASK_NEW_NAME,reply_markup=cancel_kb("edit_cancel",callback_data.kind))
...
async def edit_categories_arch(callback: CallbackQuery, callback_data: NavCB):
    await callback.message.edit_text(text=CHOOSE_ARCHIVE,reply_markup=edit_archived_kb(callback_data.kind))
    return
...
async def edit_categories_max(callback: CallbackQuery,state: FSMContext,callback_data: NavCB):
    ...
    await callback.message.edit_text(text=ASK_MAX,reply_markup=cancel_kb("edit_cancel",callback_data.kind))
...
async def archive_category(callback: CallbackQuery, db: Database, state: FSMContext, callback_data: CategoryCB,budget_id):
    if callback_data.purpose == "archive":
        ...
        await state.clear()
        text = await build_edit_screen(db, budget_id, CATEGORY_ARCHIVED, callback_data.kind)
        ...
        await state.clear()
        text = await build_edit_screen(db, budget_id, CATEGORY_DEARCHIVED,callback_data.kind)
        await callback.message.edit_text(text, reply_markup=edit_kb(callback_data.kind))
...
async def rename_category(callback: CallbackQuery, state: FSMContext,callback_data: CategoryCB):
    ...
    await callback.message.edit_text(text=ASK_NEW_NAME,reply_markup=cancel_kb("edit_cancel",callback_data.kind))
...
async def delete_category(callback: CallbackQuery,state:FSMContext,callback_data:CategoryCB):
    ...
    await callback.message.edit_text(DELETE_CONFIRM, reply_markup=confirm("confirm_delete","edit",callback_data.kind))
...
async def input_max_create(message: Message,state: FSMContext,db: Database,budget_id):
    ...
    text = await build_edit_screen(db, budget_id, CATEGORY_ADDED.format(data["category_name"]),data["kind"])
...
async def input_category_max(message: Message,state: FSMContext, db: Database, budget_id):
    ...
    text = await build_edit_screen(db, budget_id, CATEGORY_MAX,data["kind"])
...
async def input_category_name(message: Message,state: FSMContext, db: Database, budget_id):
    ...
    if await db.check_category_name(budget_id,category_name) is not None:
        ...
        await message.answer(NAME_EXISTS, reply_markup=confirm(confirm_text,"edit_cancel",kind))
        ...
        if mode == "create":
            await state.update_data(category_name=category_name)
            if kind == "expense":
                await message.answer(text=PROMPT_MAXIMUM,reply_markup=confirm("create_max","create_confirm",kind))
            else:
                await db.add_category(category_name, budget_id, 0, sort_order, kind)
                text = await build_edit_screen(db, budget_id, CATEGORY_ADDED.format(name=category_name),
                                               kind)
            ...
            await state.clear()
            text = await build_edit_screen(db, budget_id, CATEGORY_RENAMED.format(name=category_name),kind)
            await message.answer(text, reply_markup=edit_kb(kind))

@router.callback_query()
async def unhandled(callback: CallbackQuery, state: FSMContext):
    print("НЕ ПОЙМАН:", callback.data, "| состояние:", await state.get_state())
