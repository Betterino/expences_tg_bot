...
from utils import parse_amount, fit
...
from .screens import render_categories_screen,render_edit_screen,build_edit_screen,edit_rich,answer_rich
...
async def edit_categories_create(callback: CallbackQuery, state: FSMContext, callback_data: NavCB, db: Database, budget_id):
    ...
    blocks = await build_edit_screen(db, budget_id, Categories.ADDED.format(name=data["category_name"]),callback_data.kind)
    await answer_rich(callback.message, blocks, reply_markup=confirm("edit_cat","edit",callback_data.kind))
...
async def edit_categories_cat(callback: CallbackQuery,state: FSMContext, db: Database,callback_data: NavCB, budget_id):
    blocks = await build_edit_screen(db, budget_id, "",callback_data.kind)
    blocks = blocks + [richtext.para(Categories.ASK_NEW_NAME)]
    await edit_rich(callback.message, blocks, reply_markup=cancel_kb("edit_cancel",callback_data.kind))
...
async def archive_category(callback: CallbackQuery, db: Database, state: FSMContext, callback_data: CategoryCB,budget_id):
    if callback_data.purpose == "archive":
        ...
        await state.clear()
        blocks = await build_edit_screen(db, budget_id, Categories.DONE, callback_data.kind)
        await edit_rich(callback.message, blocks, reply_markup=edit_kb(callback_data.kind))
    else:
...
async def input_max_create(message: Message,state: FSMContext,db: Database,budget_id):
    ...
    blocks = await build_edit_screen(db, budget_id, Categories.ADDED.format(name=data["category_name"]),data["kind"])
    await answer_rich(message, blocks, reply_markup=confirm("edit_cat","edit",data["kind"]))
...
async def input_category_max(message: Message,state: FSMContext, db: Database, budget_id):
    ...
    blocks = await build_edit_screen(db, budget_id, Categories.MAX_CHANGED,data["kind"])
    await answer_rich(message, blocks, reply_markup=edit_kb(data["kind"]))
...
async def input_category_name(message: Message,state: FSMContext, db: Database, budget_id):
    ...
    if len(category_name) > 15:
        category_name = fit(category_name, 15)
    data = await state.get_data()
    mode = data["mode"]
    ...
    if await db.check_category_name(budget_id,category_name) is not None:
        ...
        if mode == "create":
            ...
            if kind == "expense":
                ...
                blocks = await build_edit_screen(db, budget_id, Categories.ADDED.format(name=category_name),
                                               kind)
                await answer_rich(message, blocks, reply_markup=confirm("edit_cat", "edit", kind))

        else:
            await db.rename_category(data["category_id"], category_name)
            await state.clear()
            blocks = await build_edit_screen(db, budget_id, Categories.RENAMED.format(name=category_name),kind)
            await answer_rich(message, blocks, reply_markup=edit_kb(kind))
